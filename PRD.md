...
- Maintains one `uinput` virtual keyboard via `evdev`'s uinput support; writes remapped events there. The device is declared on the `i8042` (PS/2) bus so libinput classifies it as an *internal* keyboard (via its bundled `10-generic-keyboard.quirks`, matched by bus — libinput ignores the `ID_INPUT_KEYBOARD_INTEGRATION` udev property). Because the daemon holds the real keyboard grabbed, this synthetic device is the only one libinput sees typing on, so tagging it internal is what keeps libinput's disable-while-typing (touchpad palm rejection) working.
...
- **Every keymap** also maps Super+Up and Super+Down to a synthetic Super tap: Super+Up opens GNOME's Activities overview (the expose-style view of every window) and Super+Down closes it again. (GNOME's overview is a single stateless toggle, so closing is the same Super tap as opening.)
- **Keymap-independent** (applied with Super *not* held, regardless of focused app): Ctrl+Alt+Left/Right/Up -> Super+Left/Right/Up (GNOME snap-left / snap-right / maximize); the trigger Ctrl+Alt is released so the compositor sees a clean Super+arrow and restored when the arrow releases.
- **Firefox** (`firefox.desktop`)
- **Ghostty** (`com.mitchellh.ghostty.desktop`) — Ghostty's own defaults, derived from `src/config/Config.zig` (the macOS `super+` bindings) against `ghostty +list-keybinds --default` (Linux): Super+{C,V,A,F,N,T,W,Q} -> Ctrl+Shift+{same}; Super+{`,`/`=`/`-`/0/Enter} -> Ctrl+{same}; Super+{1..9} -> Alt+{same} (tab navigation); Super+Shift+{`,`/Enter/P} -> Ctrl+Shift+{same} (reload-config / zoom-split / command-palette); new split Super+D -> Ctrl+Shift+O and Super+Shift+D -> Ctrl+Shift+E; and split navigation Super+[ / ] -> Super+Ctrl+[ / ] (goto prev/next), Super+Alt+Arrow -> Ctrl+Alt+Arrow (goto directional), Super+Ctrl+Arrow -> Super+Ctrl+Shift+Arrow (resize). A terminal has no history nav, so Super+Left/Right are swallowed (they do nothing) rather than passing the real Super through. The engine matches input modifiers (Shift/Ctrl/Alt) on physical state, so modifier press-order is irrelevant. The macOS-only natural-text-editing binds (super+arrow/backspace) and alt+super+i (inspector) have no Linux default and are omitted.
